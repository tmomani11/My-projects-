...
template <typename T>
void LazyBST<T>::writeInOrder(std::ostream& out) const {
    writeIOHelper(out, m_root);
}

template <typename T>
void LazyBST<T>::writeIOHelper(std::ostream& out, TreeNode<T>* n) const {
    if (n != nullptr) {
...
// HELPER METHODS
template <typename T>
T LazyBST<T>::get(T input) {
    return getHelper(input, m_root);
}

template <typename T>
T LazyBST<T>::getHelper(T input, TreeNode<T>* node) {
    if (node == NULL) {
        return T();
